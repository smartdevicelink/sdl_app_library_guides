...
To display a Turn by Turn direction, a combination of the @![iOS]`SDLShowConstantTBT`!@ @![android]`TODO Add Property`!@ and @![iOS]`SDLAlertManeuver`!@ @![android]`TODO Add Property`!@ RPCs must be used. The @![iOS]`SDLShowConstantTBT`!@ @![android]`TODO Add Property`!@ RPC involves the data that will be shown on the head unit. The main properties of this object to set are `navigationText1`, `navigationText2`, and `turnIcon`. A best practice for navigation applications is to use the `navigationText1` as the direction to give (Turn Right) and `navigationText2` to provide the distance to that direction (3 mi). When an @![iOS]`SDLAlertManeuver`!@ @![android]`TODO Add Property`!@ is sent, you may also include accompanying text that you would like the head unit to speak when an direction is displayed on screen (e.g. In 3 miles turn right.).
