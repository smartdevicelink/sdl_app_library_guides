...
The first difference for a navigation app is the `appHMIType` of `SDLAppHMITypeNavigation` that has to be set in the @![iOS]`SDLLifecycleConfiguration`!@ @![android]`SdlManager`!@. Navigation apps are also non-media apps.
