...
@![android, javaEE, javaSE]Media clock operations require the HMI status to be `FULL`. More information on how to monitor the HMI status can be found in the [Understanding Permissions](Getting Started/Understanding Permissions#monitoring-the-hmi-level) guide.!@
