...
### Button Presses
Another unique feature of remote control is the ability to send simulated button presses to the associated modules, imitating a button press on the hardware itself. Simply specify the module, the button, and the type of press you would like. Again, this request can fail if the module is unable to be controlled. Please make sure the module can be controlled by sending a `SDLGetInteriorVehicleDataConsent` before sending the button press RPC.
