...
@![javaEE]
Make sure that you follow the steps in [Installation](Getting Started/Installation) and [Integration Basics](Getting Started/Java Suite Integration Basics) sections to create a new JavaEE SDL project before continuing this section.
...
The following snippet is a truncated version of what is needed to set up the WebSocket server to accept and pass connections to the SDL library. This example uses the `ws` npm module for WebSocket connections. Refer to [the integration basics guide](Getting Started/JavaScript Suite Integration Basics) for the full integration setup.
